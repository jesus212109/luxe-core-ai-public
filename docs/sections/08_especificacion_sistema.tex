...
\begin{enumerate}
    ...
    \item \textbf{El Nodo Firmware (C++20):} Ejecutado en el ESP32 DevKit V1~\cite{espressif_esp32_trm}, el mismo dispositivo físico que actualmente actúa como puente BLE para el ventilador FanLamp (Sección~\ref{sec:fanlamp_evolucion}). Cuando se complete el firmware descrito en la Sección~\ref{sec:firmware_esp32}, este ESP32 se reubicará y pasará a traducir comandos de red a tramas del bus LIN~\cite{iso_17987_2025} de la máquina HVAC. \textbf{[Pendiente de implementar.]}
\end{enumerate}
...
\caption{Arquitectura de tres capas. Las flechas indican flujo de comunicación entre componentes.}
...
El sistema opera sobre el segmento de red aislado descrito en el Capítulo~\ref{cap:restricciones_metodologia}, implementado mediante un router \textbf{TP-Link TL-MR6400} sin enlace WAN que actúa como \textit{switch} Ethernet y punto de acceso WLAN.

\begin{figure}[tbp]
\centering
\includegraphics[width=\textwidth]{img/fig_8_2.pdf}
\caption{Topología de despliegue sobre la red air-gapped. Las líneas continuas indican Ethernet, las discontinuas Wi-Fi, y las punteadas BLE. El control del ventilador y el sensor se realiza a través del ESP32 como puente BLE, mientras que el enchufe Tuya se comunica directamente por IP local.}
...
Enchufe Tuya & \texttt{192.168.1.100/24} & Wi-Fi & Actuador del humidificador \\
\hline
\end{tabular}
...
\hline
001 & Entorno híbrido UCO (Conda + uv) & Cap.~5 (sección 5.2.1) & Superado por ADR-001b \\
001b & Migración a arquitectura Edge local & Cap.~5 (sección 5.2.1) & Implementado \\
002 & Aislamiento de red y Hotspot Spoofing & Cap.~5 (sección 5.2.4) & Implementado \\
003 & Lectura GATT directa de sensores Xiaomi & Cap.~5 (sección 5.2.4.2) y Cap.~8 & Implementado \\
...
006 & Infraestructura de servidor y Ubuntu Server 24.04 & Cap.~5 (sección 5.2.5) & Implementado \\
007 & Despliegue de red y contingencia NAT & Cap.~5 (sección 5.3.2) & Documentado \\
008 & Migración de CMDOP a OpenClaw npm & Cap.~4 (sección 4.6) y Cap.~8 & Implementado \\
\hline
\end{tabular}
...
\subsection{Modelos Evaluados para el Razonador (Tier~2)}
\label{sec:mejora_modelos}

Durante el desarrollo se evaluaron varios modelos como candidatos para el Tier~2 de razonamiento. La Tabla~\ref{tab:modelos_evaluados_tier2} resume los resultados de las pruebas de estrés realizadas sobre el hardware real del proyecto (Ryzen 5, 16~GB RAM). El análisis detallado de este proceso se recoge en la Sección~\ref{sec:migracion_qwen} del Capítulo~5.

\begin{table}[ht]
\centering
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{|p{2.8cm}|p{2.0cm}|p{1.6cm}|p{2.0cm}|p{3.5cm}|}
\hline
\textbf{Modelo} & \textbf{Parámetros} & \textbf{Latencia (s)} & \textbf{Thinking} & \textbf{Decisión} \\
\hline
Qwen2.5 7B & 7B & 5--30 & No & Base de partida, estable \\
Qwen3.5 4B & 4B & 45--180 & Forzado & Descartado (JSON inestable) \\
Qwen3.5 9B & 9B & 180--720 & Forzado & Descartado (inviable en CPU) \\
Mistral 7B (Q4) & 7B & 27--60 & No & \textbf{Seleccionado} \\
\hline
\end{tabular}
\caption{Modelos evaluados como candidatos para el Tier~2 de razonamiento. Las mediciones se realizaron sobre el corpus de 500 comandos reales del proyecto.}
\label{tab:modelos_evaluados_tier2}
\end{table}

La decisión final combinó Qwen2.5 1.5B para el Tier~1 (clasificación determinista, 300--500~ms) y Mistral 7B para el Tier~2 (razonamiento conversacional, 27--60~s), logrando una arquitectura mixta donde cada modelo se especializa en la tarea para la que ofrece mejor rendimiento. La arquitectura del enrutador es \textit{model-agnostic}: sustituir el modelo de razonamiento requiere únicamente modificar dos líneas en \texttt{config.py}, sin alterar el resto del sistema.
...
El motor de inferencia se apoya en \textbf{Ollama} (\texttt{localhost:11434}) con los modelos \texttt{qwen2.5-coder:1.5b} (clasificador, siempre en RAM) y \texttt{mistral:7b-instruct-q4\_K\_M} (razonador, bajo demanda), más \texttt{bge-m3}~\cite{bge_m3} para \textit{embeddings} semánticos. Toda la comunicación se mantiene dentro del segmento \textit{air-gapped}.
...
\begin{enumerate}
    ...
    \item \textbf{Tier~1 --- Clasificador 1.5B:} Utiliza el modelo \texttt{qwen2.5-coder:1.5b}~\cite{hui_qwen25_coder_2024} con un \textit{prompt} cerrado de más de 130 ejemplos categorizados (iluminación, ventilador, escenas, consultas, delegación). El modelo responde exclusivamente con JSON estructurado o el token \texttt{DELEGAR}. Parámetros optimizados: \texttt{num\_ctx=2048}, \texttt{max\_tokens=256}, \texttt{temperature=0.0}. Su generación JSON determinista es esencial para el \textit{parser} del enrutador. Latencia típica: 300--500~ms en Ryzen 5.
    \item \textbf{Tier~2 --- Razonador Mistral 7B:} Emplea el modelo \texttt{mistral:7b-instruct-q4\_K\_M}~\cite{mistral_7b}, que se carga bajo demanda con \texttt{keep\_alive=0} y se descarga automáticamente tras 5 minutos de inactividad. Ofrece respuesta directa sin \textit{thinking} forzado, con una calidad conversacional que evita la sensación de interactuar con un sistema puramente determinista. Latencia: 27--60~s. Durante la espera, el sistema muestra un indicador visual al usuario para señalizar que el razonador está procesando la consulta.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Circuit Breaker para modelos:} Patrón \textit{circuit breaker} con tres estados (CLOSED, OPEN, HALF\_OPEN) para los subsistemas de Ollama (modelos 1.5B y Mistral 7B). Si un modelo falla 3 veces consecutivas, el circuito se abre durante 30~segundos, evitando reintentos inútiles. La recuperación es automática mediante sondeo en estado HALF\_OPEN.
...
El \textbf{Sensor Daemon} (\texttt{sensor\_daemon.py}) es un servicio \texttt{systemd} que lee el sensor BLE Xiaomi LYWSD03MMC cada 60~segundos mediante conexión GATT directa desde el adaptador Bluetooth del servidor. Almacena dos salidas:
...
\subsection{Criterios de Integración y Restricciones}

La integración de cada dispositivo ha seguido dos principios:
...
\begin{itemize}
    ...
    \item \textbf{Aprendizaje mediante resolución de problemas reales.} La heterogeneidad de protocolos no es un defecto, sino una característica deliberada del proyecto. La capa de orquestación con inteligencia artificial puede unificar dispositivos de distintos fabricantes, protocolos y generaciones (incluso aquellos que no fueron diseñados para interoperar) sin modificar su hardware.
\end{itemize}

\subsection{Impacto de la Restricción Económica en las Decisiones de Implementación}
\label{sec:impacto_economico}

El presupuesto reducido del proyecto (56,50~\euro{} en hardware nuevo, 0~\euro{} en licencias) no fue un obstáculo sino un condicionante activo que moldeó cada integración. Para cada dispositivo, la falta de herramientas de análisis comerciales obligó a buscar alternativas que, en muchos casos, resultaron más instructivas que la solución rápida:

\begin{itemize}
    \item \textbf{Ventilador FanLamp:} un analizador BLE como el nRF52840 (~50~\euro{}) habría capturado el protocolo en minutos. Sin él, fue necesario decompilar el APK de la aplicación (~60~MB) con \texttt{jadx}, descubriendo que el ventilador no usa conexiones GATT, sino anuncios BLE. Este hallazgo no habría sido evidente con un analizador comercial.
    \item \textbf{Enchufe Tuya:} Un mando universal por infrarrojos o un hub Zigbee habrían simplificado la integración, pero hubieran reintroducido dependencia de un fabricante. La extracción de la \textit{Local Key} mediante \texttt{tinytuya wizard} fue gratuita y demostró que los dispositivos Tuya sí permiten control LAN completo.
    \item \textbf{Sensor Xiaomi:} Flashear el firmware ATC MiThermometer (~30~minutos de trabajo) habría dado acceso a los datos sin necesidad de conexión GATT. Pero la lectura directa de la característica \texttt{ebe0ccc1} resultó ser legible sin autenticación, eliminando cualquier riesgo de \textit{brick} y cualquier dependencia de la nube.
    \item \textbf{Puente BLE del ESP32:} El ESP32, originalmente adquirido como futuro nodo LIN para la HVAC, se reutilizó como radio BLE programable para el FanLamp, evitando la compra de un adaptador Bluetooth adicional.
\end{itemize}

En todos los casos, la restricción económica forzó un conocimiento más profundo del hardware del que se habría obtenido siguiendo la ruta convencional.

\section{Firmware del Nodo ESP32 (\texttt{firmware/})}
\label{sec:firmware_esp32}
...
El desarrollo de este firmware no se ha completado dentro del calendario del TFG por dos razones. La primera es de alcance: tras documentar el protocolo LIN del fabricante y analizar el repositorio \texttt{esphome-fujitsu-halcyon} de Omniflux~\cite{omniflux_esphome_fujitsu}, se confirmó que dicho código contiene ya el \textit{parser} de tramas LIN necesario para comunicarse con la máquina General/Fujitsu. Esto reduce drásticamente el trabajo de ingeniería inversa que sería necesario de otro modo, pero su integración con el resto del sistema (adaptación del bucle serie, gestión de errores, validación de comandos) sigue siendo una tarea que requiere semanas de desarrollo y pruebas.
...
El proyecto incluye un enchufe inteligente Tuya con medición de consumo, documentado en \texttt{firmware/Enchufe/devices.json}. La integración ha sido completamente validada en la red \textit{air-gapped}.
...
El ventilador modelo F8808 de Mikomika, adquirido en AliExpress por 35~\euro{} e instalado personalmente por el autor, es el dispositivo más complejo de integrar. La Figura~\ref{fig:fanlamp} muestra el modelo real instalado en el techo del dormitorio.
