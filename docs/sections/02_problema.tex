...
Estudios del sector~\cite{cnmc_panel_2025} muestran que una parte relevante de los hogares españoles carece de acceso a Internet de banda ancha, con diferencias significativas entre zonas urbanas y rurales. A esto se suman los inevitables cortes de suministro eléctrico, eventualidades imprevisibles que afectan a cualquier vivienda. Durante esos cortes no solo se pierde la conectividad externa, sino que los routers domésticos (que dependen de la corriente) dejan de funcionar por completo, lo que paraliza cualquier sistema domótico que dependa de servidores externos para procesar sus rutinas locales.
...
Todos estos problemas (dependencia de red, exposición de datos, latencia, fragmentación de protocolos y restricción económica) comparten una misma raíz: la delegación de la inteligencia del hogar a infraestructuras externas. Este proyecto propone una respuesta directa: una configuración de control local que integra hardware heterogéneo mediante software desacoplado y orquestado por inteligencia artificial local, sin depender de la nube para ninguna función crítica.

A partir de este diagnóstico, el capítulo siguiente define los objetivos concretos que se persiguen y los indicadores que permitirán medir su cumplimiento.
