...
\hline
\end{tabular}
\caption{Desglose de costes de hardware del proyecto Luxe Core AI.}
...
\begin{tabular}{|p{2.3cm}|p{2.0cm}|p{2.3cm}|p{3.5cm}|p{2.8cm}|}
\hline
\textbf{Componente} & \textbf{Seleccionado} & \textbf{Alternativas} & \textbf{Motivo de la elección} & \textbf{Alternativa descartada por} \\
\hline
Inferencia LLM & Ollama & llama.cpp, localAI, GPT4All & Ecosistema maduro, soporte nativo de GPU, API REST estándar~\cite{ollama} & llama.cpp requiere compilación manual; LocalAI inestable con modelos pequeños \\
\hline
Clasificador ligero & qwen2.5-coder:1.5b & phi-2, gemma-2b, tinyllama & JSON determinista, 300--500~ms en CPU, 986~MB RAM; sin \textit{thinking} forzado & phi-2 optimizado para inglés; gemma-2b requiere más VRAM \\
\hline
Razonador & mistral:7b-instruct-q4\_K\_M & llama3.2-8b, qwen3.5-9b, zephyr-7b & Respuesta directa sin \textit{thinking} forzado; 27--60~s en CPU; calidad conversacional~\cite{mistral_7b} & qwen3.5-9b inviable en CPU (3--12 min); llama3.2 necesita más contexto \\
\hline
Embeddings & bge-m3 & all-MiniLM-L6-v2, e5-mistral, jina-embeddings & Multilingüe (español+inglés), eficiente en RAM (500~MB)~\cite{bge_m3} & all-MiniLM: solo inglés; e5-mistral: demasiado pesado (7B) \\
\hline
...
El sistema completo opera sobre un router \textbf{TP-Link TL-MR6400} en configuración \textit{air-gapped}:
\begin{itemize}
