...
\end{verbatim}
    ...
    \begin{itemize}
        \item \textbf{Proveedor de IA:} permite elegir entre numerosos proveedores (Ollama local, OpenAI, Anthropic, Google, etc.). Si se selecciona Ollama, se puede escoger el modelo deseado de entre los que se descargaron previamente (por ejemplo, \texttt{mistral:7b}). Una vez funcionando, se pueden añadir más modelos y cambiar entre ellos en cualquier momento.
...
\begin{itemize}
    ...
    \item \textbf{Clasificación inteligente:} los comandos ambiguos o con variantes se analizan con un clasificador de IA ligero (1.5B parámetros) que los categoriza y ejecuta en aproximadamente 300--500~ms.
    \item \textbf{Conversación abierta:} las preguntas que requieren contexto o razonamiento se responden con Mistral 7B, un modelo conversacional que tarda entre 27 y 60~segundos en generar una respuesta.
\end{itemize}
...
\begin{itemize}
    \item ``Enciende el enchufe'' o ``Activa el humidificador''
    \item ``Apaga el enchufe'' o ``Desactiva el humidificador''
\end{itemize}
...
Enchufe & ``Enciende el enchufe'', ``Activa el humidificador'', ``Apaga el enchufe'' \\
...
El sistema está diseñado con la privacidad y la seguridad como principios fundamentales. No obstante, el usuario debe seguir unas pautas básicas para mantener la seguridad del sistema:
...
\begin{itemize}
    \item \textbf{El sistema no responde:} comprueba que el servidor está encendido ({\footnotesize\texttt{ping 192.168.1.50}}).\\ Servicios: {\footnotesize\texttt{systemctl status model-router sensor-daemon}}.\\ OpenClaw: {\footnotesize\texttt{openclaw gateway status}}.
    ...
    \item \textbf{El sensor no da lecturas:} el Sensor Daemon lee cada 60~segundos. Si lleva más de 2~minutos sin datos, reinicia el servicio: {\small\texttt{sudo systemctl restart sensor-daemon}}.\\ Si el problema persiste, comprueba la batería del sensor (voltaje inferior a 2500~mV indica batería baja).
...
\begin{itemize}
    ...
    \item \textbf{¿Puedo controlar el sistema desde fuera de mi casa?} El sistema está diseñado para funcionar exclusivamente en la red local. Para acceder desde fuera, sería necesario exponer el servicio a Internet, lo que contradice el principio de privacidad del proyecto. El canal Telegram utiliza los servidores de Telegram como intermediario, pero las decisiones de control se toman siempre en local.
...
\begin{enumerate}
    ...
    \item Instalar Ollama (3~min):\\ \url{https://ollama.ai/install.sh} (\texttt{curl -fsSL ... | bash})
    \item Descargar los modelos de lenguaje (15~min):\\ \texttt{ollama pull qwen2.5-coder:1.5b}\\ \texttt{ollama pull mistral:7b-instruct-q4\_K\_M}\\ \texttt{ollama pull bge-m3}
...
\begin{enumerate}
    ...
    \item Instalar OpenClaw (3~min):\\ \url{https://openclaw.ai/install.sh} (\texttt{curl -fsSL ... | bash})
