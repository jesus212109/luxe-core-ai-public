...
\begin{frame}{Auto-Recuperación y Tolerancia a Fallos}
  ...
  \IfFileExists{figures/17_recuperacion.pdf}{%
    \includegraphics[width=0.92\textwidth, height=4.3cm, keepaspectratio]{figures/17_recuperacion.pdf}%
  }{
  ...
  \begin{exampleblock}{Flujo de recuperación}
    \scriptsize
    \textbf{SelfHealManager} verifica cada 30\,s la salud de Ollama,
    Circuit Breaker y ESP32. Ante un fallo:
    \begin{enumerate}
      \item \textbf{Circuit Breaker} aísla el servicio (\texttt{CLOSED} $\to$ \texttt{OPEN})
